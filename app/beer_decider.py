...
class BeerDecider(KnowledgeEngine):
    ...
    def pale_ale(self):
        beer = Beer(nombre="Pale Ale", color="Rubia", maltas=["pale", "caramel", "munich"], lupulo=["cascade", "centennial"], levadura=["ale"], maceracion="Verter 15 litros de agua en el macerador. Macerar la malta pale durante 1 hora a 65 ºC para obtener el mosto.",
                    ebullicion="Añadir agua al mosto hasta tener 27 litros. Aumentar a temperatura de ebullición y añadir los lúpulos cuando se indica. La ebullición durará una 1 hora y 10 minutos.", fermentacion="Fermentar a unos 18 ºC con levadura US-05 de fermentación alta para ales americanas.", acondicionamiento="Madurar a 12 ºC durante 10 días. Cebar la cerveza con azúcar y guardarla durante seis semanas a 18 ºC.")
        self.declare(beer)
    ...
    def imperial_porter(self):
        beer = Beer(nombre="Imperial Porter", color="Negra", maltas=["varies"], lupulo=["varies"], levadura=["ale"], maceracion="Se maceran las seis maltas en 19,2 litros de agua durante 1 hora a 67 ºC para obtener el mosto.", ebullicion="Añadir agua al mosto hasta obtener 27 litros y hervir durante 1 hora y 10 minutos. Llevar a ebullición y añadir el lúpulo en los momentos indicados anteriormente.",
                    fermentacion="Se fermenta el mosto con levadura de fermentación baja a unos 12 ºC.", acondicionamiento="Se debe guardar la cerveza fermentada durante 11 semanas a 12 ºC para que madure y se equilibre el carbónico.")
        self.declare(beer)
    ...
    def amber_ale(self):
        beer = Beer(nombre="Amber Ale", color="Roja", maltas=["english Pale Ale", "american two-row", "crystal", "victory"], lupulo=["horizon", "cascade", "centennial"], levadura=["ale"], maceracion="Macerar las maltas durante 60 minutos a 65 ºC. Usar 20 litros de agua.",
                    ebullicion="Lavar y recircular hasta obtener el mosto. Añadir agua hasta alcanzar los 27 litros. Hervir durante 60 minutos, añadiendo los lúpulos tal y como se explica anteriormente.", fermentacion="Añadir levadura de Ale americana y fermentar a 18 – 20 ºC.", acondicionamiento="Madurar durante 5 semanas a 12 ºC.")
        self.declare(beer)
    ...
    def blonde_ale(self):
        beer = Beer(nombre="Blonde Ale", color="Rubia", maltas=["american two-row", "crystal"], lupulo=["williamette"], levadura=["lagger", "ale"], maceracion="Macerar las maltas durante 60 minutos a 65 ºC. Usar 16,25 litros de agua.",
                    ebullicion=" Lavar y recircular hasta obtener el mosto. Añadir agua hasta alcanzar los 27 litros. Hervir durante 70 minutos, añadiendo los lúpulos tal y como se explica anteriormente.", fermentacion="Añadir levadura de abadía y fermentar a unos 22 ºC.", acondicionamiento="Madurar durante 6 semanas a 12 ºC.")
        self.declare(beer)

    @ Rule(Beer(nombre=MATCH.beer_nombre, color=MATCH.beer_color, maltas=MATCH.beer_maltas, lupulo=MATCH.beer_lupulo, levadura=MATCH.beer_levadura, maceracion=MATCH.beer_maceracion, ebullicion=MATCH.beer_ebullicion, fermentacion=MATCH.beer_fermentacion, acondicionamiento=MATCH.beer_acondicionamiento))
    def chosen_beer(self, beer_nombre, beer_color, beer_maltas, beer_lupulo, beer_levadura, beer_maceracion, beer_ebullicion, beer_fermentacion, beer_acondicionamiento):
        self.beers.append(Beer(nombre=beer_nombre, color=beer_color,
                          maltas=beer_maltas, lupulo=beer_lupulo, levadura=beer_levadura, maceracion=beer_maceracion, ebullicion=beer_ebullicion, fermentacion=beer_fermentacion, acondicionamiento=beer_acondicionamiento))
